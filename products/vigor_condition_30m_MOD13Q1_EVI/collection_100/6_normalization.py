...
def calcHistogram(ndviDs, ndviFreq, totalPixels, XChunk):
	...
	for x in range(0,Xsize,XChunk):
		...
		#ndvi = ndviBand.ReadAsArray(x,0,XChunk,YSize).astype(np.float);
		ndvi = ndviBand.ReadAsArray(x,0,XChunk,YSize).astype(float);
		#mask = maskBand.ReadAsArray(x,0,XChunk,YSize);
		...
		#maskFilter = np.logical_not(np.equal(ndvi,np.zeros(ndvi.shape)))
		#maskFilter = np.where(ndvi ==0 , True, False)
		#maskFilter = np.logical_not(ndvi==0)

		#plt.imshow(maskFilter)
		#plt.colorbar()
		#plt.show()
...
ndviFreq = dict(sorted(ndviFreq.items()))

uniqValues = list(ndviFreq.keys())
...
for ndviFilepath in input_files:
	...
	for x in range(0,Xsize,XChunk):
		...
		#maskFiler = np.logical_not(np.equal(ndvi,np.zeros(ndvi.shape)))
		#np.logical_not(np.nonzero(ndvi))
...
#EumapAllRasters -  Percs:[144. 169.], Min: 50.0, Max: 200.0, Avg. Below 1%: 140.375, Avg. Above 99%: 171.75
#LAPIG_Norm -  Percs:[? ?], Min: 50.0, Max: 200.0, Avg. Below 1%: 97.469, Avg. Above 99%: 109.71